...
namespace bwi_directions_generator {
    ...
    MapInfo BwiDirectionsGenerator::GenerateDirectionsForPathOnMap(std::vector<geometry_msgs::PoseStamped> path, fs::path mapFile, std::string destinationName, std::string floorStr) {
		...
		return MapInfo(translator, path, destinationName, floorStr);
    }
}